...
class track:
    ...
    def __init__(self):
        ...
        self.car_img = im.imread('carfig.png')
    ...
    def plot_world(self):
        ...
        if not self.ferrari == None:
            figure(200)
            clf()
            velImg = zeros((self.ferrari.n_actions, self.ferrari.n_vel_size, self.ferrari.n_vel_size))
            posImg = zeros((self.ferrari.n_actions, self.ferrari.n_pos_size, self.ferrari.n_pos_size))
            velImgTrace = zeros((self.ferrari.n_actions, self.ferrari.n_vel_size, self.ferrari.n_vel_size))
            posImgTrace = zeros((self.ferrari.n_actions, self.ferrari.n_pos_size, self.ferrari.n_pos_size))
            for a in arange(self.ferrari.n_actions):
               velImg[a,:,:] = flipud(self.ferrari.weights[a,:self.ferrari.n_states_vel].reshape(self.ferrari.n_vel_size,self.ferrari.n_vel_size))
               posImg[a,:,:] = flipud(self.ferrari.weights[a,self.ferrari.n_states_vel:].reshape(self.ferrari.n_pos_size,self.ferrari.n_pos_size))
               velImgTrace[a,:,:] = flipud(self.ferrari.eligibility_trace[a,:self.ferrari.n_states_vel].reshape(self.ferrari.n_vel_size,self.ferrari.n_vel_size))
               posImgTrace[a,:,:] = flipud(self.ferrari.eligibility_trace[a,self.ferrari.n_states_vel:].reshape(self.ferrari.n_pos_size,self.ferrari.n_pos_size))

            velImgBig = zeros((sqrt(self.ferrari.n_actions) * self.ferrari.n_vel_size, sqrt(self.ferrari.n_actions) * self.ferrari.n_vel_size))
            posImgBig = zeros((sqrt(self.ferrari.n_actions) * self.ferrari.n_pos_size, sqrt(self.ferrari.n_actions) * self.ferrari.n_pos_size))
            velImgBigTrace = zeros((sqrt(self.ferrari.n_actions) * self.ferrari.n_vel_size, sqrt(self.ferrari.n_actions) * self.ferrari.n_vel_size))
            posImgBigTrace = zeros((sqrt(self.ferrari.n_actions) * self.ferrari.n_pos_size, sqrt(self.ferrari.n_actions) * self.ferrari.n_pos_size))

            dirs = ((1,1),(1,0),(2,0),(2,1),(2,2),(1,2),(0,2),(0,1),(0,0))
            a = 0
            for d in dirs:
                dx = d[0]
                dy = d[1]
                velImgBig[dx*self.ferrari.n_vel_size:(dx+1)*self.ferrari.n_vel_size, dy*self.ferrari.n_vel_size:(dy+1)*self.ferrari.n_vel_size] = velImg[a,:,:]
                posImgBig[dx*self.ferrari.n_pos_size:(dx+1)*self.ferrari.n_pos_size, dy*self.ferrari.n_pos_size:(dy+1)*self.ferrari.n_pos_size] = posImg[a,:,:]
                velImgBigTrace[dx*self.ferrari.n_vel_size:(dx+1)*self.ferrari.n_vel_size, dy*self.ferrari.n_vel_size:(dy+1)*self.ferrari.n_vel_size] = velImgTrace[a,:,:]
                posImgBigTrace[dx*self.ferrari.n_pos_size:(dx+1)*self.ferrari.n_pos_size, dy*self.ferrari.n_pos_size:(dy+1)*self.ferrari.n_pos_size] = posImgTrace[a,:,:]
                a = a + 1

            subplot(221)
            im1 = imshow(velImgBig, interpolation='nearest', cmap='jet')
            subplot(222) 
            im2 = imshow(posImgBig, interpolation='nearest', cmap='jet')
            subplot(223)
            im1 = imshow(velImgBigTrace, interpolation='nearest', cmap='jet')
            subplot(224) 
            im2 = imshow(posImgBigTrace, interpolation='nearest', cmap='jet')
            # subplot(223)
            # im3 = imshow(self.ferrari.velAct.reshape(self.ferrari.n_vel_size,self.ferrari.n_vel_size), interpolation='nearest', cmap='jet')
            # subplot(224)
            # im4 = imshow(self.ferrari.posAct.reshape(self.ferrari.n_pos_size,self.ferrari.n_pos_size), interpolation='nearest', cmap='jet')
    ...
    def move(self,action): 
        ...
        #if(self.finished):
        #    t = self.time / 1000.0
        #    rew = 4*self.max_rew*exp(-5 * t) + self.max_rew
